...
# S033 — Meeting B — mock match reps
...
| Session ID | S033 |
| Title | Meeting B — mock match reps |
| Calendar date | 2027-01-15 (planning input; 4:00–6:00 PM unless Kickoff/event) |
| Meeting type | B |
...
- Documented progress on the focus named in `calendar.yaml` `title` for S033
- No new active features without evidence

## Prerequisites

- [calendar.yaml](../calendar.yaml) row for S033
...
**Filename is fixed:** `S033-meeting-b.md`. Edit `title` in this file and `calendar.yaml` when the week's topic changes.
