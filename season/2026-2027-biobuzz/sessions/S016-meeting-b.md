...
# S016 — Meeting B — driver and auto reps
...
| Session ID | S016 |
| Title | Meeting B — driver and auto reps |
| Calendar date | 2026-11-06 (planning input; 4:00–6:00 PM unless Kickoff/event) |
| Meeting type | B |
...
- Documented progress on the focus named in `calendar.yaml` `title` for S016
- No new active features without evidence

## Prerequisites

- [calendar.yaml](../calendar.yaml) row for S016
...
**Filename is fixed:** `S016-meeting-b.md`. Edit `title` in this file and `calendar.yaml` when the week's topic changes.
